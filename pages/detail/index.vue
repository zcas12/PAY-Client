<template>
  <el-container class="contents">
    <div class="prd_detail_box renew">
      <div class="left_area">
        <div class="prd_img">
          <img alt="상세 이미지" :src="product.src"/>
        </div>
      </div>
      <div class="right_area">
        <div class="prd_info">
          ...
          <div class="price">
            <span class="price-1">
              <strong>{{product.price | comma}}</strong>
              <span>원</span>
            </span>
          </div>
...
export default {
  ...
  computed:{
    ...
    product(){
      return this.$_.find(this.cosmetics,(item)=>item.code===this.productId)
    },
    totalPrice(){
      const result = this.product?.price * this.num
